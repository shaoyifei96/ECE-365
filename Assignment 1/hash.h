...
class hashTable {
 ...
 public:
  ...
  // 2 if rehash fails.
  int insert(const std::string &key, void *pv = NULL);//done

  // Check if the specified key is in the hash table.
  // If so, return true; otherwise, return false.
  bool contains(const std::string &key);//done
 ...
 private:
  ...
  //custome made function!!!!!! for calc hash and hash two remainers
  void get_hash(int &pos, int &shift, const std::string &key);
  ...
  static unsigned int getPrime(int size);//done
};
